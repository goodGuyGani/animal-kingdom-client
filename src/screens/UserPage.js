...
function UserPage() {
  ...
  useEffect(() => {
    Axios.get("https://graceful-nightgown-wasp.cyclic.app/api/lessons").then(
      (response) => {
  ...
  useEffect(() => {
    Axios.get(
      `https://graceful-nightgown-wasp.cyclic.app/api/checkchapterstate`
    ).then((response) => {
  ...
  const logout = () => {
    Axios.get("https://graceful-nightgown-wasp.cyclic.app/api/logout").then(
      (response) => {
        navigate("/");
      }
    );
  ...
  useEffect(() => {
    if (isInitialMount55.current) {
      ...
      Axios.get(
        `https://graceful-nightgown-wasp.cyclic.app/api/starcheck/${userId}`
      ).then((response) => {
    ...
    <>
      ...
      <center>
        <h2 className="head-text">
          My Profile<span style={{ color: "#4c7031" }}></span>
        </h2>
      </center>
      ...
      <div className="container1">
        {!profilePic == "" ? (
          <img
            ...
            src={`https://graceful-nightgown-wasp.cyclic.app/api/images/${profilePic}`}
            alt="profile"
          />
        ) : (
          <img
        ...
        <div className="p-text1">
          ...
          <h2 className="head-text" style={{ fontSize: 20 }}>
            <span style={{ color: "black" }}>Username: </span>
            {user}
            <span style={{ color: "#4c7031" }}></span>
          </h2>
          <h2 className="head-text" style={{ fontSize: 20 }}>
          ...
          <h2 className="head-text" style={{ fontSize: 20 }}>
            <span style={{ color: "black" }}>Animal Captured: </span>
            {animalCap}
            <span style={{ color: "#4c7031" }}></span>
          </h2>
        </div>
        ...
        <div class="cards">
          {chapterState
            ...
            .map((val) => (
              <div
                class="cards-inner"
                key={val.chapterStateId}
                onClick={() => {
                  navigate(
                    `/chapter/${val.chapterStateLessonId}/${val.chapterStateChapterId}/${userId}`
                  );
                }}
              >
                <div style={{ marginLeft: 10 }}>
                  {lesson
                    .filter((lesson) => {
                      return userId === ""
                        ? val
                        : lesson.lessonId == val.chapterStateLessonId;
                    })
                    .map((val) => (
                  ...
                  {lesson
                    ...
                    .map((val) => (
                      <span style={{ fontSize: 14, color: "#46364a" }}>
