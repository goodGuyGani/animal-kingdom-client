...
function Quiz() {
  ...
  useEffect(() => {
    Axios.get(
      `https://graceful-nightgown-wasp.cyclic.app/api/quiz2/${lessonid}`
    ).then((response) => {
  ...
  useEffect(() => {
    if (isInitialRef2.current) {
      ...
      Axios.get(
        `https://graceful-nightgown-wasp.cyclic.app/api/quiz/${lessonid}/${quiznum}`
      ).then((response) => {
  ...
  // https://graceful-nightgown-wasp.cyclic.app

  const submitAnswer = (answer) => {
    if (answer === "") {
      setMessage("Please Pick an Answer");
    } else if (answer === correctAnswer) {
      Axios.post("https://graceful-nightgown-wasp.cyclic.app/api/answers", {
        userId: myId,
        lessonId: lessonid,
        quizNum: quiznum,
        answerState: "Correct",
                ...
                <div class="quiz-header">
                  ...
                  <div style={{ marginTop: 20 }}>
                    <button
                      ...
                      style={{ margin: 8 }}
                      onClick={() => {
                        submitAnswer(`${val.optionA}`);
                        {
                          quiznum == 10
                            ? navigate(`/quizresult/${myId}/${lessonid}`)
                            : navigate(
                                `/quizpage/${lessonid}/${val.quizNum + 1}`
                              );
                        }
                      }}
                    >
